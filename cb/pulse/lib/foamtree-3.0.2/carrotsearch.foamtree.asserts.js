...
(function() {
  ...
  var optionDefinitions = function() {
    ...
    args["initializer"] = {
      ...
      asserts: And(NotEmpty(), OneOf([
        "fisheye", "treemap", "blackhole", "order", "random"
      ]))
    };
  ...
  if (foamtreeClass) {
    ...
    for (var assert in args) {
      if (args.hasOwnProperty(assert) && !args[assert].static) {
 ...
 * -----------------
 *
 * Build type    : Carrot Search FoamTree HTML5 (commercial variant)
 * Build version : 3.0.2
 * Build number  : FOAMTREE-SOFTWARE0-DIST-11
 * Build time    : Feb 14, 2014
 * Built by      : bamboo
 * Build revision: bugfix/3.0.x/121fc42c
 */