function sentiment_datacall(client_id, dom_id) {
    var url = "https://app.netbase.com/cb/pulse/api/request"
	+ "?id=" + client_id 
	+ "&dataSpec=" + JSON.stringify(sentiment_dataSpec);
	...
	$.ajax({
	    ...
	    success: function(data, textStatus, jqXHR) {
	        ...
	        window.setTimeout(function() {
	            sentiment_datacall(client_id, dom_id);
	        }, jsResult.waitTime);

	        // if (currentValueTime !== previousValueTime) {
	        	sentiment_handleUpdate(jsResult, dom_id); //function to repeat rendering of charts
	        // }
	        console.log('**********************************************')
