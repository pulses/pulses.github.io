...
// 	viewerSetup();
// 	datacall();
// 	// <!-- initX("bars_1", "af0a69e5-1989-4dc2-a697-7d5a2e13eaba");                 
//     // initX("bars_2", "4c633aa4-12c9-4298-b634-c8b6f3e810dd"); -->
...
function handleUpdate(data,dom_id) {
	...
	var msie = ua.indexOf("MSIE ");
    		...
    		var chartSentiment = differenceChart("#"+dom_id+"-sparkline")
    		...
    		// function callSentimentSparkline2() {
    		// 	d3.select("#div2-sparklineThree")
    		// 	.datum(sentimentSparklineData)
    		// 	.call(chartSentiment);
    		// }
    		
    		//callSentimentSparkline(dom_id+"-sparkline")
    		callSentimentSparkline("#"+dom_id+"-sparkline");
    		//callSentimentSparkline2();
			...
			var sentimentTicker = new countUp(dom_id+"-uniqueAuthorsTotal", prefixScale_SecondLastSentiment, prefixScale_LastSentiment, 0, 15)//data.waitTime)//10);
...
sentimentTicker.start();
			...
			var avgsentimentTicker = new countUp(dom_id+"-uniqueAuthorsAvg", avgSentimentPrevious, avgSentiment, 0, 15)//10);
...
var avgDiffSentimentTicker = new countUp(dom_id+"-avgDiffSentiment", sentimentAvgDifferenceBefore, sentimentAvgDifferenceNow, 0, 15);

avgDiffSentimentTicker.start();

$('#'+dom_id+'-avgDiffSentiment').css('color', function(){
...
$('#'+dom_id+'-1avgDiffSentimentPercentSign').css('color', function(){
	if( sentimentAvgDifferenceNow > 0) {
		return "#2ECC40"
	} else if( sentimentAvgDifferenceNow < 0) {
		return "#FF4136"
