...
(function() {
  ...
  window.randomGroups = function (count) {
    var arr = [];
  ...
  $(document).ready(function() {
    ...
    $('.api[id]').each(function(i, e) {
      ...
      var specs = [];
      if (attr) {
        if (attr.type)       { specs.push("@type " + attr.type); }
        if (attr.asserts)    { specs.push("@assert " + attr.asserts); }
        if (attr.since)      { specs.push("@since " + attr.since); }
        if (attr.deprecated) { specs.push("@deprecated " + attr.deprecated); }
      } else {
  ...
  $(window).load(function() {
    if (document.location.hash) {
      ...
      $sidebar.scrollTop($("#toc a[href='" + document.location.hash + "']").offset().top -
